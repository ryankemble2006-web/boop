...
resumes its deployment. The v156 Now Playing artist link targets Shield only, after local TV-emulator checks. The v155 favourites selector is user-accepted. The selector belongs to BOOP and does not require Nvidia Home. The v153 corners, 10-second seeking, Down-to-Pause and 250 ms hold are user-approved and must be preserved. No permission
...
Startup Manager/defaults to be combined on `boop-unified-v146-integration` and installed on
Pixel 10 Pro XL and Shield as one permanent-signed APK. All input branches and
standalone Animation Lab remain preserved. See SESSION_HANDOFF.md.
