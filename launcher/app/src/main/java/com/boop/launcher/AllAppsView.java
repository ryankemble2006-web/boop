...
public final class AllAppsView extends FrameLayout {
 public interface Listener{void launch(AppEntry app);void pin(AppEntry app);void searchOpened();void searchClosed();void closeDrawer();}
 private final GridView grid;private final ImageButton searchButton;private final EditText search;private final Listener listener;private List<AppEntry> all=new ArrayList<>();private List<AppEntry> shown=new ArrayList<>();private float swipeDownY;private boolean swipeStartedAtTop;
 public AllAppsView(Context c,Listener listener){super(c);this.listener=listener;setBackgroundColor(Color.BLACK);setContentDescription("All apps");
  ...
  grid.setOnTouchListener((v,event)->{int action=event.getActionMasked();if(action==MotionEvent.ACTION_DOWN){swipeDownY=event.getY();swipeStartedAtTop=grid.getFirstVisiblePosition()==0&&(grid.getChildCount()==0||grid.getChildAt(0).getTop()>=grid.getPaddingTop()-dp(2));return false;}if(action==MotionEvent.ACTION_UP&&swipeStartedAtTop&&event.getY()-swipeDownY>dp(72)){listener.closeDrawer();return true;}return false;});
