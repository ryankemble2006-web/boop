...
public final class MainActivity extends Activity {
 ...
 @Override protected void onResume(){super.onResume();EdgeToEdge.hideBars(this);if(widgets.prune(items)){store.save(items);if(workspace!=null)workspace.refresh();}showLauncherImmediately();}
 @Override protected void onStop(){widgets.stop();super.onStop();}
 @Override protected void onNewIntent(Intent i){super.onNewIntent(i);setIntent(i);state=LauncherState.HOME;showState(false);EdgeToEdge.hideBars(this);showLauncherImmediately();}
 ...
 private void openWall(){android.util.Log.i("BOOPLauncher","BOOP_WALL_SWIPE");Intent i=getPackageManager().getLaunchIntentForPackage("com.boop.alpha1");if(i==null){Toast.makeText(this,"BOOP Wall is not installed",Toast.LENGTH_SHORT).show();return;}i.addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT|Intent.FLAG_ACTIVITY_NEW_TASK|Intent.FLAG_ACTIVITY_NO_ANIMATION);try{EdgeToEdge.hideBars(this);startActivity(i);overridePendingTransition(0,0);}catch(Exception e){Toast.makeText(this,"BOOP Wall could not open",Toast.LENGTH_SHORT).show();}}
 private void showLauncherImmediately(){if(root==null)return;root.animate().cancel();root.setAlpha(1f);}
