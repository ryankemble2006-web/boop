...
- [ ] Publish: use separate workflow .github/workflows/build-shield-launcher.yml with existing BOOP signing secrets, no replacement signer; upload BOOP-Launcher.apk plus source/hash/signer receipts.
- [ ] Verify exact signed artifact package/version/components/signature, document physical-test limits, update handoff/status/memory and shared main routing only, scoped commit/push and live HEAD verification.
