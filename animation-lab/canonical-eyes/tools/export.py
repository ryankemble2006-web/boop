...
(out/'README.md').write_text('# Canonical BOOP eye animation transplant\n\n26 code-driven animations share one state engine, original eye texture, and shader. Copy runtime/java plus runtime/assets into the target host; keep its existing renderer lifecycle/permissions. Integrate CanonicalEyeRenderer via GLSurfaceView and supply EyeMotion.Pose from EyeMotion.Controller. Source/README.md documents the contract. The demonstration Activity is optional lab-only source. No Android profile, voice, media authority or notification policy is included.\n\nThis replaces eye-motion implementation; it does not replace accessory artwork or claim to implement hands. It has not been promoted into Unified. No extra bottom eyelid or whole-image jiggle. Default master bytes are locked.\n')
