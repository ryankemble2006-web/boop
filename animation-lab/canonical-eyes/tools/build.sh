...
"$TOOLS/apksigner" sign --ks "$BOOP_SIGNING_STORE_FILE" --ks-key-alias boop-dev --ks-pass env:BOOP_DEV_STORE_PASSWORD --key-pass env:BOOP_DEV_KEY_PASSWORD --out "$OUT/BOOP-Animation-Lab-v8.apk" "$OUT/aligned.apk"
"$TOOLS/apksigner" verify --print-certs "$OUT/BOOP-Animation-Lab-v8.apk" > "$OUT/signer.txt"
grep -Fq 'f5af40378ef06445b43f6001ae602fc18ce16eefbabdefd23afe178a47b5cdde' "$OUT/signer.txt"
"$TOOLS/aapt" dump badging "$OUT/BOOP-Animation-Lab-v8.apk" > "$OUT/badging.txt"
grep -Fq "package: name='com.boop.animationlab' versionCode='8'" "$OUT/badging.txt"
...
sha256sum "$OUT/BOOP-Animation-Lab-v8.apk" > "$OUT/apk-sha256.txt"
python - "$OUT/BOOP-Animation-Lab-v8.apk" <<'PY'
