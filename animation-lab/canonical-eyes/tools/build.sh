...
"$TOOLS/apksigner" sign --ks "$BOOP_SIGNING_STORE_FILE" --ks-key-alias boop-dev --ks-pass env:BOOP_DEV_STORE_PASSWORD --key-pass env:BOOP_DEV_KEY_PASSWORD --out "$OUT/BOOP-Animation-Lab-v13.apk" "$OUT/aligned.apk"
"$TOOLS/apksigner" verify --print-certs "$OUT/BOOP-Animation-Lab-v13.apk" > "$OUT/signer.txt"
grep -Fq 'f5af40378ef06445b43f6001ae602fc18ce16eefbabdefd23afe178a47b5cdde' "$OUT/signer.txt"
"$TOOLS/aapt" dump badging "$OUT/BOOP-Animation-Lab-v13.apk" > "$OUT/badging.txt"
grep -Fq "package: name='com.boop.animationlab' versionCode='13'" "$OUT/badging.txt"
...
for permission in INTERNET POST_NOTIFICATIONS FOREGROUND_SERVICE FOREGROUND_SERVICE_DATA_SYNC; do grep -Fq "uses-permission: name='android.permission.${permission}'" "$OUT/badging.txt"; done
if grep '^uses-permission:' "$OUT/badging.txt" | grep -Ev "android.permission.(INTERNET|POST_NOTIFICATIONS|FOREGROUND_SERVICE|FOREGROUND_SERVICE_DATA_SYNC)'"; then echo 'Unexpected permission' >&2; exit 1; fi
git rev-parse HEAD > "$OUT/built-commit.txt"
sha256sum "$OUT/BOOP-Animation-Lab-v13.apk" > "$OUT/apk-sha256.txt"
python - "$OUT/BOOP-Animation-Lab-v13.apk" <<'PY'
