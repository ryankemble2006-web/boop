...
## Flow shortcut from v236 (source candidate)

The Now Playing Close player slot is now labelled Flow and requests the selected native Deezer session's existing Flow URI directly, without leaving BOOP. Preserve the130x44dp slot, existing margins/focus and accent styling. Separate Close media remains. No cold-launch/Cast fallback: unsupported native URI control reports unavailable. The new route is independent of, and does not modify, the accepted v235 favourite/dislike helper. Ten focused tests passed; signed build/install/physical acceptance pending source publication. See docs/handoffs/2026-09-20-shield-flow-button-v236.md.