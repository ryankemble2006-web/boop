...
An earlier247 Activity check using the same callback resumed Home159 ms after accepted playback without queued speech. Final249 debugger-injected Activity checks were inconclusive and are not final UI acceptance; Ryan physically retested the Shield mic command and accepted the return timing as "fast enough". The white-circle follow-up was identified as the Shield SystemUI assistant launch orb, shown before BOOP opens. No supported suppression switch was found; removal remains unresolved and no system/remapping change was applied. Final APKs and receipts are retained in task outputs and Desktop/APKBOOP. Details: `docs/handoffs/2026-09-21-music-mic-return-v249.md`.
