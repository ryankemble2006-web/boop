...
public final class SharedEyeColourHarness {
    ...
    public static void main(String[] args) {
        for (int hue = 0; hue < 360; hue++) {
            check(SharedEyeColourProtocol.decode(SharedEyeColourProtocol.encode(hue)) == hue,
                    "Every saved hue must round-trip exactly");
        }
        for (String bad : new String[]{"", "190", "BOOP_EYE_V1|360", "BOOP_EYE_V1|-1",
                "BOOP_EYE_V1|1.5", "BOOP_EYE_V1|NaN", "BOOP_EYE_V1|0190",
                "BOOP_EYE_V1|190\n", "BOOP_EYE_V2|190", "unknown", "unavailable", null}) {
            check(SharedEyeColourProtocol.decode(bad) == null, "Reject invalid shared value");
        }
        rejects(() -> SharedEyeColourProtocol.encode(360), "Never clamp remote errors to a colour");
        rejects(() -> SharedEyeColourProtocol.encode(-1), "Never encode out-of-range hue");
        check(SharedEyeColourProtocol.isOwnedHelper(SharedEyeColourProtocol.MARKER, 0, 32, "text", false),
                "Recognize only the marked BOOP helper");
        check(!SharedEyeColourProtocol.isOwnedHelper(".*", 0, 32, "text", false), "No name-only helper claim");
        check(!SharedEyeColourProtocol.isOwnedHelper(SharedEyeColourProtocol.MARKER, 0, 32, "text", true),
                "Initial values must not defeat HA restart restoration");
        check(!SharedEyeColourProtocol.isOwnedHelper(SharedEyeColourProtocol.MARKER, 0, 2, "text", false),
                "Reject wrong helper shape");
        check(SharedEyeColourProtocol.chooseHelper(List.of()) == null, "Missing helper is not invented");
        check("owned".equals(SharedEyeColourProtocol.chooseHelper(List.of("owned"))), "Use returned helper ID");
        rejects(() -> SharedEyeColourProtocol.chooseHelper(List.of("a", "b")), "Ambiguous helper is not guessed");
        ...
        System.out.println("SharedEyeColourHarness: " + checks + " checks passed");
    }
}
