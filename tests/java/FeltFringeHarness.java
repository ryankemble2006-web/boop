...
public final class FeltFringeHarness {
 ...
 public static void main(String[] args)throws Exception {
  ...
  check(mesh.length>1000 && mesh.length%42==0,"Actual artwork needs triangle fringe geometry");
  check(Arrays.equals(mesh,FeltFringeMesh.create(pixels,w,h,guide,rw,rh)),"Fibres must be stable across context recreation");
  int outside=0,left=0,right=0,surface=0;
  for(int i=0;i<mesh.length;i+=7){
   float x=mesh[i]+mesh[i+2],y=mesh[i+1]+mesh[i+3],light=mesh[i+4],alpha=mesh[i+5];
   if(alpha>.05){
    float widthPx=2*(float)Math.hypot(mesh[i+2]*w/2,mesh[i+3]*h/2);
    check(widthPx>=1.0f&&widthPx<=3.3f,"Soft fibre ribbon has a readable pixel width");
    for(float scale:new float[]{.25f,.57f,1f,2f}){
     float screenWidth=2*(float)Math.hypot(mesh[i+2]/scale*(w*scale)/2,mesh[i+3]/scale*(h*scale)/2);
     check(Math.abs(screenWidth-widthPx)<.001f,"Screen width survives phone/preview/TV scale");
    }
   }
   ...
   if(alpha>.05 && (pixels[py*w+px]>>>24)>=128)surface++;
   if(alpha>.05 && (pixels[py*w+px]>>>24)<32){
    outside++;if(px<w/2)left++;else right++;
    boolean near=false;
    for(int dy=-24;dy<=24&&!near;dy++)for(int dx=-24;dx<=24;dx++){
  ...
  check(surface>300,"Visible wisps lie on the felt crown as well as outside it");
  ...
  System.out.println("PASS actual-master fringe: "+mesh.length/7+" vertices, "+outside+" outside-alpha and "+surface+" surface vertices; readable pixel widths across four scales");
 }
}
