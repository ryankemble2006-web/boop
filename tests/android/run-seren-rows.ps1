...
if (-not $OutputDirectory) { $OutputDirectory = [IO.Path]::GetFullPath((Join-Path $RepoRoot '../seren-row-audit')) }
...
$Manifest = (Get-Content -Raw -LiteralPath "$PSScriptRoot/seren-rows-manifest.xml").Replace('com.boop.shieldoverlay', $Package)
[IO.File]::WriteAllText("$OutputDirectory/AndroidManifest.xml", $Manifest)
Invoke-Checked 'javac' @('-encoding','UTF-8','-source','17','-target','17','-cp',$AndroidJar,'-d',"$OutputDirectory/classes","$PSScriptRoot/SerenRowsProbe.java")
