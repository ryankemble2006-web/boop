...
if (-not $OutputDirectory) { $OutputDirectory = [IO.Path]::GetFullPath((Join-Path $RepoRoot '../notification-fixture')) }
...
$Manifest = (Get-Content -Raw -LiteralPath "$PSScriptRoot/notification-fixture-manifest.xml")
[IO.File]::WriteAllText("$OutputDirectory/AndroidManifest.xml", $Manifest)
Invoke-Checked 'javac' @('-encoding','UTF-8','-source','17','-target','17','-cp',$AndroidJar,'-d',"$OutputDirectory/classes","$PSScriptRoot/NotificationFixture.java")
...
Invoke-Checked $Adb @('-s',$Serial,'shell','pm','grant','local.boop.notificationfixture','android.permission.POST_NOTIFICATIONS')
Write-Output 'Installed offline notification fixture on verified emulator'
