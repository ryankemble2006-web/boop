...
def test_shield_release_keeps_aligned_weather_in_v253():
    gradle = (ROOT / "split/shield/build.gradle").read_text()
    verify = (ROOT / "split/verify-apks.py").read_text()
    assert "versionCode 253" in gradle
    assert "versionName '1.2.253-shield'" in gradle
    assert "version = 253 if body == 'shield' else 220" in verify
