...
def test_shield_release_keeps_aligned_weather_in_v248():
    gradle = (ROOT / "split/shield/build.gradle").read_text()
    verify = (ROOT / "split/verify-apks.py").read_text()
    assert "versionCode 248" in gradle
    assert "versionName '1.2.248-shield'" in gradle
    assert "version = 248 if body == 'shield' else 215" in verify
