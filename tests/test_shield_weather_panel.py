...
def test_shield_release_keeps_aligned_weather_in_v250():
    gradle = (ROOT / "split/shield/build.gradle").read_text()
    verify = (ROOT / "split/verify-apks.py").read_text()
    assert "versionCode 250" in gradle
    assert "versionName '1.2.250-shield'" in gradle
    assert "version = 250 if body == 'shield' else 217" in verify
