...
def test_shield_release_keeps_aligned_weather_in_v244():
    gradle = (ROOT / "split/shield/build.gradle").read_text()
    verify = (ROOT / "split/verify-apks.py").read_text()
    assert "versionCode 244" in gradle
    assert "versionName '1.2.244-shield'" in gradle
    assert "version = 244 if body == 'shield' else 211" in verify
