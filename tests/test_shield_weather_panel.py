...
def test_shield_release_keeps_aligned_weather_in_v242():
    gradle = (ROOT / "split/shield/build.gradle").read_text()
    verify = (ROOT / "split/verify-apks.py").read_text()
    assert "versionCode 242" in gradle
    assert "versionName '1.2.242-shield'" in gradle
    assert "version = 242 if body == 'shield' else 209" in verify
