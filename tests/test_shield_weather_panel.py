...
def test_shield_release_keeps_aligned_weather_in_v232():
    gradle = (ROOT / "split/shield/build.gradle").read_text()
    verify = (ROOT / "split/verify-apks.py").read_text()
    assert "versionCode 232" in gradle
    assert "versionName '1.2.232-shield'" in gradle
    assert "version = 232 if body == 'shield' else 207" in verify
