...
def test_shield_release_keeps_aligned_weather_in_v243():
    gradle = (ROOT / "split/shield/build.gradle").read_text()
    verify = (ROOT / "split/verify-apks.py").read_text()
    assert "versionCode 243" in gradle
    assert "versionName '1.2.243-shield'" in gradle
    assert "version = 243 if body == 'shield' else 210" in verify
