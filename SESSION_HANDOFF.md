...
## v125 signed GitHub receipt, 2026-09-11

GitHub Unified run 34650157297 completed successfully for exact source fe5d4b8b3b0d5496e69e58d38d7e954456661d55, version125 / 1.2.125-canonical-animation-integration. The workflow passed nonvisual integration, canonical animation transplant, shared-state/media, Startup Manager prevention/exact Undo, notification/dev-lab/natural-voice, wake handoff, Launcher source, Shield control, wake/routing/lifecycle and permanent-signing gates. Signed APK assembly, package/version/entry/activity checks, permanent-signer verification, ZIP integrity and artifact upload all passed. Shield focused tests: 68, Unified focused tests: 217, zero failures/errors/skips. APK SHA256: 03f731aa7ff85a745a55c98284db400ccdd92c6927501fe4f1c9902fbc62e323. Artifact BOOP-Unified ID 10283976036; artifact ZIP digest sha256:1b0c15b752b48223fdb1027ebdf4c94ed6f7d77ef367f2ca9f3651e17de24a2b. Visual/device acceptance remains manual and pending.
## v125 Shield integration candidate, 2026-09-11

Ryan requested the completed Startup Manager line and prepared animation transplant be reconciled into one Shield/Unified continuation and bumped to v125. This branch starts from live `boop-shield-startup-cleanup-v121` commit `00017612d379e9e228f0340c06ae33d329c7ac5f`, then restores canonical commit `6bfc2e63eaf80395503afca43e80983efa5066d0` so audio mode decisions read the Shield's live sample-rate toggle instead of trusting stale process memory. The audio conflict was resolved by retaining v122 foreground/Cast arbitration while removing `lastMode` caching and querying `AudioManager.getParameters` before writes.

The animation payload comes from frozen prep commit `1df79da85b47890575324471520cbb984233749d`: canonical catalogue/runtime, developer preview activity/menu/router, exact idle/double-blink contracts, Freddie/sign motion, locked eye/hand integrity checks, materialization and non-visual CI gates. Three pre-existing dirty Shield Home/Settings files in the prep worktree were explicitly excluded as unrelated concurrent work. v125 version is `125 / 1.2.125-canonical-animation-integration`. Ryan remains the visual authority; no GitHub visual checks are added or run. Fresh local v125 verification: materialization completed; 4 direct Python integration contracts passed; StartupCleanupPolicyTest, StartupPreventionPolicyTest, StartupPreventionRecordTest and all 5 canonical animation/blink Java contracts passed; canonical core checks passed; forced Android unit suite passed (277 tests, 0 failures, 0 errors, 0 skipped). Local assemble compiled through validateSigningDebug then stopped because the private boopDev keystore is intentionally not configured locally; signing/package-integrity verification is delegated to the existing GitHub stable-signing workflow. No signing configuration was changed.
...
Ryan approved a native-Deezer-only `Lyrics` button in Unified Shield Now Playing. This branch was replayed onto published Startup Manager v2 base `c7a38b7`, so the candidate preserves that v128 work and advances to version `131` / `1.2.131-deezer-lyrics-shortcut`.

Clean physical probing showed Deezer's player exposes a unique descendant `content-desc="Lyrics"` inside a clickable parent; no fixed screen coordinate is stored. BOOP opens the current Deezer notification target, resolves that semantic control from a fresh UI hierarchy, clicks its live bounds, then requires Deezer's scrolling lyric panel before reporting success. Non-Deezer sessions do not show the button. Existing non-Deezer Close-player geometry/focus behavior is preserved.

Fresh candidate verification before publication: canonical core checks pass including Lyrics target/panel and native-Deezer policy regressions; materialized Unified Android unit suite is 277 tests, zero failures/errors/skips. Permanent-signed GitHub build, installation and physical BOOP-button acceptance remain pending at this entry.
