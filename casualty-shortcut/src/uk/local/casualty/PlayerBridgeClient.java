...
final class PlayerBridgeClient {
    ...
    private final Messenger replies=new Messenger(new Handler(Looper.getMainLooper()) {
        @Override public void handleMessage(Message message) {
            ...
            if(message.what==13) {
                Toast.makeText(context,"Approve the Android debugging prompt for the Casualty shortcut.",Toast.LENGTH_LONG).show();
                return;
            }
    ...
    void prepare(Callback result) {
        ...
        try {
            Intent intent=new Intent().setComponent(new ComponentName("uk.local.casualty","uk.local.casualty.PlayerControlService"));
