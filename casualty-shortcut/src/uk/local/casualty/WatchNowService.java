...
public final class WatchNowService extends AccessibilityService {
    ...
    private void report(String state) {
        if(!state.equals(lastState)) { lastState=state; Log.i("Casualty",state); }
    }
        ...
        try {
            ...
            try {
                ...
                if(gate.claimProfile(now,page.chooser,page.profile!=null)) {
                    ...
                    Log.i("Casualty","First iPlayer profile click accepted: "+clicked);
                    report("First profile selected; waiting for Casualty");
                    return;
                }
                if(!page.title) {
                    report(page.chooser ? "Waiting for first existing iPlayer profile" : "Waiting for Casualty title; nodes: "+page.visited);
                    return;
                }
