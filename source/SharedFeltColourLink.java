...
final class SharedFeltColourLink {
    ...
    SharedFeltColourLink(Channel channel, Listener listener, IntSupplier currentHue) {
    ...
    void begin(boolean mayCreate) {
        ask("input_text/list", new JSONObject(), result -> {
            helperId = SharedFeltColourHaProtocol.findHelper(result);
            if (helperId == null && mayCreate) create();
            else if (helperId == null) fail("Sharing needs setup. Your local felt colour is unchanged.");
            else resolve(() -> read(true));
        });
    }
    private void create() {
        try {
            ask("input_text/create", SharedFeltColourHaProtocol.createBody(), result -> {
                ...
                ask("input_text/list", new JSONObject(), list -> {
                    helperId = SharedFeltColourHaProtocol.findHelper(list);
    ...
    private void resolve(Runnable then) {
        ask("config/entity_registry/list", new JSONObject(), list -> {
            String next = SharedFeltColourHaProtocol.findEntity(list, helperId);
    ...
    void event(String entity, String value) {
        ...
        if (!ready) return;
        Integer hue = SharedFeltColourProtocol.decode(value);
        if (hue == null) fail("Shared felt colour is unavailable. Keeping this device's last colour.");
        else listener.colour(hue);
    }
    void write(int hue) {
        SharedFeltColourProtocol.requireHue(hue);
        if (!active || !ready) return;
        ask("input_text/list", new JSONObject(), list -> {
            if (!helperId.equals(SharedFeltColourHaProtocol.findHelper(list))) {
    ...
    private void setValue(int hue, boolean first) {
        try {
            JSONObject body = new JSONObject().put("domain", "input_text").put("service", "set_value")
                    ...
                    .put("service_data", new JSONObject().put("value", SharedFeltColourProtocol.encode(hue)));
    ...
    private void read(boolean first) {
        ...
        ask("get_states", new JSONObject(), result -> {
            Integer hue = SharedFeltColourHaProtocol.readHue(result, entityId);
            if (eventVersion != readAt) hue = SharedFeltColourProtocol.decode(lastEvent);
            if (hue == null && first && createdId != null && !seeded) {
                seeded = true; setValue(currentHue.getAsInt(), true); return;
            }
            if (hue == null) { fail("Shared felt colour is not ready. Your local colour is unchanged."); return; }
