...
final class HomeAssistantGeneralAssistantClient {
    ...
    HomeAssistantGeneralAssistantClient(SecureTokenStore tokenStore, HomeAssistantAuth auth) {
    ...
    synchronized CommandOutcome ask(String text) {
        ...
        try {
            ...
            return CommandOutcome.assistantReply(reply.speech());
                ...
                public void onMessage(WebSocket webSocket, String text) {
                    try {
                        ...
                        if ("auth_invalid".equals(type)) {
                            finish(finished, result, done, DiscoveryResult.authRequired());
                        ...
                        if ("result".equals(type)
                            ...
                            List<HomeAssistantOpenCodeAgentSelector.Agent> agents =
                                    parseAgents(message.opt("result"));
                            String selected = HomeAssistantOpenCodeAgentSelector.select(agents);
                            finish(
                                    finished,
                                    result,
                                    done,
    ...
    private static List<HomeAssistantOpenCodeAgentSelector.Agent> parseAgents(Object rawResult) {
        ...
        List<HomeAssistantOpenCodeAgentSelector.Agent> agents = new ArrayList<>();
        if (array == null) {
            return agents;
        }
        ...
        for (int i = 0; i < array.length(); i++) {
            ...
            agents.add(new HomeAssistantOpenCodeAgentSelector.Agent(
                    agent.optString("id", ""),
