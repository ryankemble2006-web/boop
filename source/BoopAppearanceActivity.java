...
/** Small shared settings surface; no extra face or animation renderer is created. */
public final class BoopAppearanceActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) {
        ...
        column.setPadding(dp(28), dp(24), dp(28), dp(24)); column.setBackgroundColor(Color.BLACK);
        scroll.addView(column);
        text(column, "Eyes and animation", 28);
        text(column, "Only the iris colour changes. BOOP's approved artwork stays intact.", 18);
        hueLabel = text(column, "Eye colour", 20);
        hue = new SeekBar(this); hue.setMax(359); hue.setKeyProgressIncrement(5);
        hue.setContentDescription("Eye colour hue"); hue.setProgress(BoopEyeHue.loadHue(this));
        ...
        share = button(column, "Share eye colour: Off", () -> {
            if (sharing.enabled()) sharing.setEnabled(false); else confirmSharing();
        });
        retry = button(column, "Retry sharing", this::confirmSharing);
        // Device-local animation speed controls are added here after colour verification.
    ...
    private void refresh() {
        if (hue == null) return;
        if (!dragging) hue.setProgress(BoopEyeHue.loadHue(this));
        hueLabel.setText("Eye colour: " + BoopEyeHue.loadHue(this));
        ...
        retry.setVisibility(sharing.enabled() && !sharing.ready() ? android.view.View.VISIBLE : android.view.View.GONE);
    }
    @Override protected void onStart() { super.onStart(); unwatch = sharing.observe(this::refresh); }
    @Override protected void onStop() { if (unwatch != null) { unwatch.run(); unwatch = null; } super.onStop(); }
    ...
    private Button button(LinearLayout column, String title, Runnable action) {
        Button view = new Button(this); view.setText(title); view.setTextSize(20); view.setMinHeight(dp(60));
        view.setOnClickListener(v -> action.run()); column.addView(view); return view;
    }
