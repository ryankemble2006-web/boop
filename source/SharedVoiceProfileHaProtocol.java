...
/** Explicit HA helper identity for the shared BOOP voice profile. */
final class SharedVoiceProfileHaProtocol {
    private SharedVoiceProfileHaProtocol() { }

    static JSONObject createBody() throws JSONException {
        return new JSONObject().put("name", SharedVoiceProfileProtocol.NAME)
                .put("min", 0).put("max", 64).put("mode", "text")
                .put("pattern", SharedVoiceProfileProtocol.MARKER);
    }
    ...
    static String findEntity(Object result, String helperId) {
        ...
        for (int i = 0; i < list.length(); i++) {
            ...
            if (entity != null) throw new IllegalArgumentException("Ambiguous voice entity");
            entity = candidate;
        }
        if (entity == null) throw new IllegalArgumentException("Voice helper is not available");
        return entity;
    }

    static SharedVoiceProfileProtocol.Profile readProfile(Object result, String entity) {
        ...
        for (int i = 0; i < list.length(); i++) {
            ...
            if (found) throw new IllegalArgumentException("Duplicate voice state");
            found = true;
            value = state.optString("state", "");
        }
        return SharedVoiceProfileProtocol.decode(value);
    }
}
