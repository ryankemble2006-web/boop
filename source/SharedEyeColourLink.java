...
final class SharedEyeColourLink {
    ...
    SharedEyeColourLink(Channel channel, Listener listener, IntSupplier currentHue) {
    ...
    void begin(boolean mayCreate) {
        ask("input_text/list", new JSONObject(), result -> {
            helperId = SharedEyeColourHaProtocol.findHelper(result);
            if (helperId == null && mayCreate) create();
            else if (helperId == null) fail("Sharing needs setup. Your local eye colour is unchanged.");
            else resolve(() -> read(true));
        });
    }
    private void create() {
        try {
            ask("input_text/create", SharedEyeColourHaProtocol.createBody(), result -> {
                ...
                ask("input_text/list", new JSONObject(), list -> {
                    helperId = SharedEyeColourHaProtocol.findHelper(list);
    ...
    private void resolve(Runnable then) {
        ask("config/entity_registry/list", new JSONObject(), list -> {
            String next = SharedEyeColourHaProtocol.findEntity(list, helperId);
    ...
    void event(String entity, String value) {
        ...
        if (!ready) return;
        Integer hue = SharedEyeColourProtocol.decode(value);
        if (hue == null) fail("Shared eye colour is unavailable. Keeping this device's last colour.");
    ...
    void write(int hue) {
        ...
        ask("input_text/list", new JSONObject(), list -> {
            if (!helperId.equals(SharedEyeColourHaProtocol.findHelper(list))) {
    ...
    private void setValue(int hue, boolean first) {
        try {
            JSONObject body = new JSONObject().put("domain", "input_text").put("service", "set_value")
                    ...
                    .put("service_data", new JSONObject().put("value", SharedEyeColourProtocol.encode(hue)));
    ...
    private void read(boolean first) {
        ...
        ask("get_states", new JSONObject(), result -> {
            Integer hue = SharedEyeColourHaProtocol.readHue(result, entityId);
            if (eventVersion != readAt) hue = SharedEyeColourProtocol.decode(lastEvent);
            if (hue == null && first && createdId != null && !seeded) {
                seeded = true; setValue(currentHue.getAsInt(), true); return;
            }
            if (hue == null) { fail("Shared eye colour is not ready. Your local colour is unchanged."); return; }
