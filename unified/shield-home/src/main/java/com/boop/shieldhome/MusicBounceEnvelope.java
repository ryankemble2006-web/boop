...
public final class MusicBounceEnvelope {
    ...
    public float update(float level, long nowMs) { return update(level, nowMs, false); }

    public float updateFast(float level, long nowMs) { return update(level, nowMs, true); }

    private float update(float level, long nowMs, boolean fast) {
        ...
        double tauMs = target > height ? (fast ? 4.0 : 28.0) : (fast ? 90.0 : 140.0);
