...
public final class StartupLocalBridge {
    ...
    public String openDeezerLyrics() throws Exception {
        ...
        return withAdb(false, null, adb -> {
            ...
            if (target == null) {
                checked(adb, "input keyevent 20");
                Thread.sleep(180L);
                screen = dumpDeezerScreen(adb);
                target = screen.lyricsTarget();
            }
            if (target == null) throw new IOException("Deezer control is not available.");
