...
public final class WatchNowService extends AccessibilityService {
    ...
    private static final class Page {
        boolean chooser,title;
        AccessibilityNodeInfo profile,episode,trailer;
        int visited;
    }

    private void report(String state) {
        if(!state.equals(lastState)) { lastState=state; Log.i("EastEnders",state); }
    }
    ...
    private void scan(AccessibilityNodeInfo node,Page page,int depth,boolean newestRow) {
        ...
        if(visible) {
            ...
            if(page.profile==null && node.isEnabled() && node.isClickable()
    ...
    static void prepareColdStart(PlayerBridgeClient.Callback callback) {
        ...
        self.stop();
        self.preparing=true;
        self.expectedPackage=PlayerReset.PLAYER;
        PlayerBridgeClient client=new PlayerBridgeClient(self,self::stop);
        self.bridge=client;
        client.prepare((ok,error) -> {
            ...
            if(ok) {
                self.lastState=null;
                self.gate.arm(SystemClock.elapsedRealtime());
    ...
    private void inspect() {
        ...
        try {
            ...
            try {
                ...
                if(gate.claimProfile(now,page.chooser,page.profile!=null)) {
                    boolean clicked=page.profile.performAction(AccessibilityNodeInfo.ACTION_CLICK);
                    Log.i("EastEnders","First iPlayer profile click accepted: "+clicked);
                    report("First profile selected; waiting for EastEnders");
                    return;
                }
                if(!page.title) {
                    report(page.chooser ? "Waiting for first existing iPlayer profile" : "Waiting for EastEnders title; nodes: "+page.visited);
                    return;
                }
                report("EastEnders title found; waiting for newest episode");
                if(gate.claimEpisode(now,true,page.episode!=null)) {
