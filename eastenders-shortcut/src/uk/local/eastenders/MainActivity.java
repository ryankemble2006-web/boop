...
public final class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) {
        ...
        Button retry = button(root, "Watch EastEnders", v -> openProgramme());
        ...
        Button setup = button(root, WatchNowService.ready() ? "Auto-play settings" : "Enable auto-play", v -> {
            ...
            if (!launch(new Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS))) {
                enabling = false;
                status.setText("Open Shield Settings, then Accessibility, and enable EastEnders auto-play.");
            }
        });
        button(root, "Close", v -> finish());
        setContentView(root);
        status.setText("If iPlayer did not open EastEnders, try the browser.");
        retry.requestFocus();
    ...
    private void openProgramme() {
        ...
        status.setText(WatchNowService.ready()
                ? "Opening EastEnders. If iPlayer ignores the link, press Back and try the browser."
                : "Opening EastEnders. To select the existing iPlayer profile and newest episode automatically, choose Enable auto-play and enable EastEnders auto-play in Accessibility settings.");
