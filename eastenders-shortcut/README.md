# EastEnders 1.8 — Shield iPlayer shortcut
...
The existing `EastEnders auto-play` accessibility helper must remain enabled in Shield Accessibility settings. No new permission grant or setup change is required for an update. If disabled, a short message explains where to enable it. If official iPlayer cannot launch, the shortcut shows a message and closes; it does not show a browser selector.
